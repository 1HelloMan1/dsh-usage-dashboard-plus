...
const Config = z.object({
  ...
  */
  ...
  /**
  * 从"今日花费"估算中排除的 provider 标识。默认排除 opencode-go，
  * 使估算只反映官方 DeepSeek API 的消耗（其它服务商的用量不经官方计费）。
  */
  excludeProviders: z.array(z.string()).default(["opencode-go"]),
  /**
...
function computeUsage(sessionsRoot, prices, schedule, scanWindowMs, externalUsageLog, excludeProviders) {
  const now = Date.now();
  ...
  for (const { path, mtimeMs } of logs) {
    ...
    for (const line of text.split("\n")) {
      ...
      if (ev.type === "request/header" && ev.data?.header?.config) {
        ...
        if (typeof cfg.provider === "string") currentProvider = cfg.provider;
        continue;
      }
      if (ev.type === "request/context" && ev.data?.provider != null) {
        if (typeof ev.data.provider === "string") currentProvider = ev.data.provider;
        continue;
      }
      if (ev.type !== "assistant/message" || ev.data?.usage == null) continue;
      if (excludeProviders.includes(currentProvider)) continue;
  ...
  if (externalUsageLog !== "off") {
    ...
    try {
      ...
      for (const line of data.split("\n")) {
        ...
        if (excludeProviders.includes("opencode-go")) continue;
...
function apply(ctx, config) {
  ...
  const todayUsage = () => {
    ...
    const value = computeUsage(sessionsRoot, prices, schedule, config.scanWindowMs, config.externalUsageLog ?? "", config.excludeProviders ?? ["opencode-go"]);
